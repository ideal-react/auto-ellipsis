...
var AutoEllipsis = (function (_React$Component) {
	...
	_createClass(AutoEllipsis, [{
		...
		value: function computeContent() {
			...
			var parentBottom = parent.getBoundingClientReact().bottom;

			var range = document.createRange();
			range.selectNodeContents(dom);
			var bottom = range.getBoundingClientReact().bottom;
			...
			if (bottom > parentBottom) {
				...
				while (endPoint >= 0) {
					...
					bottom = range.getBoundingClientReact().bottom;
					if (bottom <= parentBottom) {
						if (endPoint - 3 > 0) {
		...
		value: function render() {
			...
			return _react2['default'].createElement(this.props.tag, { style: style }, this.state.content);
		}
	}], [{
		key: 'propTypes',
		value: {
